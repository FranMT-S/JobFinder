...
const description = ref(DOMPurify.sanitize(props.jobScrapeated.job.description));

watch(() => props.jobScrapeated, () => {
  description.value = DOMPurify.sanitize(props.jobScrapeated.job.description);
})
            ...
            <v-container  class="!w-full sticky top-0 z-10  left-0 bg-surface">
              <v-row class="align-center px-4 pb-0 pt-2">
                <v-col cols="10" class="!p-0">
                  <v-card-title class="!p-0 text-h6 font-weight-medium capitalize">{{ jobScrapeated.job.position }}</v-card-title>
                </v-col>
                <v-col cols="2" class="text-right ">
                  <v-icon color="primary" class="cursor-pointer hover:scale-110" @click="$emit('update:isActiveDialog', false)">mdi-close</v-icon>
                </v-col>
              </v-row>
